...
(function () {
  ...
  function removeLegacy() { document.querySelectorAll('#rubyGiftFloat,#rubyGiftModal').forEach(el => el.remove()); }
  ...
  function ensureUI() {
    ...
    if (!gift) {
      ...
      gift.id = 'rubyDailyGift'; gift.type = 'button'; gift.setAttribute('aria-label', 'Daily Bonus');
    ...
    if (!timer) {
      timer = document.createElement('div'); timer.id = 'rubyDailyCountdown';
      timer.style.cssText = 'position:fixed;right:8px;bottom:54px;z-index:99999;min-width:94px;padding:6px 10px;border-radius:999px;background:#fff;color:#6d28d9;font-size:11px;font-weight:900;text-align:center;display:none;pointer-events:none;box-shadow:0 4px 16px rgba(0,0,0,.18);';
  ...
  function render(isEligible) {
    eligible = !!isEligible;
    const gift = ensureUI(); const timer = document.getElementById('rubyDailyCountdown');
    if (!gift || !timer) return;
    if (!loggedIn) { gift.style.display = 'none'; timer.style.display = 'none'; return; }
    gift.style.display = 'flex';
    if (eligible) {
      gift.disabled = false; gift.style.pointerEvents = 'auto'; gift.style.opacity = '1'; timer.style.display = 'none';
    } else {
      gift.disabled = true; gift.style.pointerEvents = 'none'; gift.style.opacity = '.62'; timer.style.display = 'block';
  ...
  async function refresh() {
    const client = db(); if (!client || busy) return;
    busy = true;
    try {
      const { data: s } = await client.auth.getSession(); const user = s?.session?.user; loggedIn = !!user;
  ...
  async function claim() {
    const client = db(); const gift = ensureUI();
    if (!client || !gift || gift.disabled || !loggedIn || !eligible) return;
    gift.disabled = true; gift.style.pointerEvents = 'none';
    try {
      const { data, error } = await client.rpc('claim_daily_bonus', { p_bonus: BONUS, p_cooldown_seconds: COOLDOWN });
      if (error) throw error;
      const row = Array.isArray(data) ? data[0] : data;
      // The database is authoritative. The function returns the exact next_claim_at.
      ...
      // Always render the locked state after a successful/duplicate claim response.
      render(false);
    } catch (e) { console.warn('Daily bonus claim:', e); await refresh(); }
  }

  function start() {
    ensureUI(); const client = db(); if (!client) return;
    ...
    setInterval(() => {
      ...
      if (remaining <= 0) { nextClaimAt = null; refresh(); }
      else if (timer) { timer.style.display = 'block'; timer.textContent = `⏳ ${fmt(remaining)}`; }
    }, 1000);
  }
