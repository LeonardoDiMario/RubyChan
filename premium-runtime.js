(()=>{
  const boot=()=>{
    if(document.documentElement.dataset.rubyPremiumRuntimeV2==='1')return;
    document.documentElement.dataset.rubyPremiumRuntimeV2='1';

    const style=document.createElement('style');
    style.id='ruby-premium-runtime-v2-css';
    style.textContent=`
      /* Keep Energy + Gems visible globally, without restoring the old Ruby Chan header. */
      ...
      .ruby-current-plan-badge{cursor:default}
      ...
      .ruby-plan-row{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;padding:13px;border:1px solid rgba(242,182,216,.16);border-radius:16px;background:linear-gradient(180deg,rgba(34,20,40,.9),rgba(20,13,27,.92))}
    ...
    const updatePlanBadge=()=>{
      ...
      if(!root)return;
      const badge=root.querySelector('.settings-plan-badge');
      ...
      const strong=plan.querySelector('.ruby-current-plan-name');
      if(strong)strong.textContent=cur.name;
      const small=plan.querySelector('.ruby-current-plan-copy');
      if(small)small.textContent=cur.id==='free'?'Manage your Ruby Chan membership.':cur.price+' · Premium access';
    };
    ...
    const getPage=()=>{
      ...
      if(!p){
        ...
        p.querySelector('.ruby-pay-back').onclick=()=>p.classList.remove('open');
      }
      return p;
    };
    ...
    const showCheckout=id=>{
      ...
      /* Selecting a plan changes the live badge immediately. Payment activation should still be server-verified. */
      localStorage.setItem('ruby_selected_plan',x.id);
      updatePlanBadge();
    ...
    const renderPlans=()=>{
      const p=getPage();
      p.querySelector('#rubyPayPlans').innerHTML=plans.map(x=>'<div class="ruby-pay-plan"><div class="ruby-pay-badge '+x.cls+'"><b>'+x.mark+'</b></div><div><div class="ruby-pay-name">'+x.name+'</div><div class="ruby-pay-price">'+x.price+'</div><div class="ruby-pay-copy">'+x.desc+'</div></div><button class="ruby-upgrade" data-plan="'+x.id+'">UPGRADE</button></div>').join('');
      p.querySelectorAll('[data-plan]').forEach(b=>b.onclick=()=>showCheckout(b.dataset.plan));
    };

    const openPayment=id=>{const p=getPage();renderPlans();p.classList.add('open');p.querySelector('#rubyPayCheckout').innerHTML='';if(id)setTimeout(()=>showCheckout(id),40)};
    ...
    const enhanceSettings=()=>{
      ...
      if(plan){
        ...
        if(view){
          if(view.parentElement!==plan)plan.appendChild(view);
          view.classList.add('ruby-settings-viewplans');
          view.textContent='VIEW PLANS';
          if(!view.dataset.bound){view.dataset.bound='1';view.onclick=()=>openPayment()}
        }
        const main=plan.querySelector('div:first-child');
        if(main){
          const strong=main.querySelector('strong');
          const small=main.querySelector('small');
          strong?.classList.add('ruby-current-plan-name');
          small?.classList.add('ruby-current-plan-copy');
        }
        updatePlanBadge();
      }
    };
